...
def main_menu_kb():
    s = _get_btn_styles()
    return _jkb([
        [_btn("📡  My APIs",     cb="menu:apis"),   _btn("➕  Add API",     cb="menu:add",  style=s.get('add') or None)],
        [_btn("📊  Stats",       cb="menu:stats"),   _btn("📜  Recent OTPs", cb="menu:recent")],
        [_btn("⚙️  Settings",   cb="menu:settings"), _btn("📨  Send Test",  cb="menu:test", style=s.get('add') or None)],
        [_btn("📶  API Status",  cb="menu:status")],
    ])


def back_kb(target="main"):
    return _jkb([[_btn("🔙  Back", cb=f"back:{target}")]])
...
def settings_kb():
    return _jkb([
        [_btn("🤖  Change Bot Token",       cb="set:token")],
        [_btn("💬  Manage OTP Chat IDs",    cb="set:chatid")],
        [_btn("⏱  Change Polling Interval", cb="set:interval")],
        [_btn("🎨  Button Colors",          cb="set:btnstyle", style="primary")],
        [_btn("🔙  Back",                   cb="back:main")],
    ])
...
def btnstyle_overview_kb(styles):
    ...
    for _group_title, keys in _BTN_GROUPS:
        for key in keys:
            s = styles.get(key, '')
            label = _STYLE_LABEL.get(s, '⚪ Default')
            rows.append([_btn(f"{_BTN_NAME[key]}  [{label}]", cb=f"btnstyle:pick:{key}", style=s or None)])
    rows.append([_btn("🔙  Back", cb="menu:settings")])
    return _jkb(rows)
...
def chatids_menu_kb(chat_ids):
    ...
    for i, cid in enumerate(chat_ids):
        ...
        rows.append([
            _btn(f"📋 {short}", cb=f"chatid:copy:{i}"),
...
def edit_api_kb(api_id):
    s = _get_btn_styles()
    return _jkb([
        [_btn("🔄  Re-Login (Auto)",  cb=f"api:relogin:{api_id}", style=s.get('add') or None)],
        [_btn("🧪  Test SMS",         cb=f"api:testsms:{api_id}", style=s.get('copy') or None)],
        [_btn("✏️  Edit Name",        cb=f"edit:name:{api_id}")],
        [_btn("🍪  Edit PHPSESSID",   cb=f"edit:sessid:{api_id}")],
        [_btn("🔑  Edit Sesskey",     cb=f"edit:sesskey:{api_id}")],
        [_btn("🌐  Edit Referer",     cb=f"edit:referer:{api_id}")],
        [_btn("🔗  Edit Data URL",    cb=f"edit:url:{api_id}")],
        [_btn("🔙  Back to APIs",     cb="menu:apis")],
    ])
...
def status_kb():
    return _jkb([
        [_btn("🔄 Refresh", cb="menu:status", style="primary"), _btn("🔙 Back", cb="back:main")],
    ])
...
def create_bot():
    ...
    def cb_menu(call):
        ...
        if action == 'apis':
            ...
            text = (
                ...
                f"\n⏱ Polling Interval: <b>{cfg['settings'].get('polling_interval', 1)}s</b>\n\n"
    ...
    def cb_settings(call):
        ...
        prompts = {
            ...
            'interval': "⏱ Send the <b>new polling interval</b> in seconds:\n\n<i>e.g. 0.5 = very fast, 5 = relaxed. Default: 1</i>",
        }
...
def _send_test_message():
    ...
    inline_keyboard = {
        'inline_keyboard': [
            [{**{'text': '📋 Tap to Copy OTP', 'copy_text': {'text': '123456'}}, **_s('copy')}],
            [{**{'text': '🤖 Bot Link', 'url': 'https://t.me/RTX_ABIR_BOT'}, **_s('bot_link')}],
            [{**{'text': '📢 Channel',  'url': 'https://t.me/RTX_ABIR_4090'}, **_s('channel')}],
        ]
    }
